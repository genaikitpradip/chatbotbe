...
class FileProcessor:
    ...
    async def _detect_file_type(self, file_path: str, filename: str) -> str:
        """Detect file type using multiple methods with fallbacks"""
        
        # Method 1: Try filetype library (most reliable, no system dependencies)
        try:
            kind = filetype.guess(file_path)
            if kind:
                logger.info(f"[filetype] detected: {kind.mime}")
                return kind.mime
        except Exception as e:
            logger.warning(f"[filetype] failed to detect file type: {e}")
        ...
        if MAGIC_AVAILABLE and magic:
            try:
                file_type = magic.from_file(file_path, mime=True)
                if file_type:
                    logger.info(f"[magic] detected: {file_type}")
                    return file_type
            except Exception as e:
                logger.warning(f"[magic] failed to detect file type: {e}")
        
        # Method 3: Fallback to mimetypes based on file extension
        try:
            file_type, _ = mimetypes.guess_type(file_path)
            if file_type:
                logger.info(f"[mimetypes] detected: {file_type}")
                return file_type
        except Exception as e:
            logger.warning(f"[mimetypes] failed to detect file type: {e}")
        
        # Method 4: Manual file extension mapping for common types
        ...
        if ext in extension_mapping:
            detected_type = extension_mapping[ext]
            logger.info(f"[extension mapping] detected: {detected_type}")
            return detected_type
        
        # Final fallback
        logger.warning(f"Could not detect file type for {filename}, using generic type")
        return "application/octet-stream"

    async def process_file(self, file_path: str, filename: str) -> Tuple[str, str]:
        """Process uploaded file and extract content summary"""
        try:
            file_type = await self._detect_file_type(file_path, filename)
